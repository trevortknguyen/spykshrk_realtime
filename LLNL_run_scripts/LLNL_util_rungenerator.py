    ...
    with open('%s/slurm/slurm_sub.sh'%dir_name, 'w') as fid0:
        for day, ep in zip(days, eps):
            for i0 in range(num_shift):
                run_id+=1
                with open('%s/slurm/slurm_%s'%(dir_name, run_id), 'w') as fid1:
                    fid1.write('#!/bin/bash\n')
...
if __name__=="__main__":
    ...
    parser.add_argument('-d', action='store', dest='dayepstring', help='List of days and eps to process in format d1_e1,d1_e2,d2_e1 etc')
    ...
    parser.add_argument('-r', action='store', dest='run_hours', type=int, help='Number of hours of node time to request')
    ...
    main(results.path_base_rawdata, results.spykshrk_path, results.rat_name, results.dayepstring, results.path_out, results.num_shift, results.run_hours)
